...
/**
 * A helper function to wrap Gemini API calls with consistent error handling and JSON parsing.
 * @param action The name of the action being performed (for logging).
 * @param geminiCall A function that performs the actual Gemini API call and returns the raw text response.
 * @returns The parsed JSON response.
 */
async function callGemini(action: string, geminiCall: () => Promise<string>): Promise<any> {
    try {
        const rawText = await geminiCall();
        return parseJsonResponse(rawText);
    } catch (err) {
        console.error(`Gemini API call for action '${action}' failed:`, err);
        if (err instanceof Error) {
            // Check for common network errors or API-specific messages
            if (err.message.toLowerCase().includes('fetch')) {
                 throw new Error("خطا در اتصال به سرور Gemini. لطفاً اتصال اینترنت خود را بررسی کرده و دوباره تلاش کنید.");
            }
             // Check for API key errors
            if (err.message.toLowerCase().includes('api key not valid')) {
                throw new Error("کلید API نامعتبر است. لطفاً پیکربندی برنامه را بررسی کنید.");
            }
            // Pass through other errors, including the JSON parsing error
            throw err;
        }
        throw new Error('یک خطای ناشناخته در ارتباط با سرور Gemini رخ داد.');
    }
}

export const analyzeImage = async (base64Image: string): Promise<AnalysisResult> => {
    return callGemini('analyzeImage', async () => {
        const prompt = `
...
export const getMorphSuggestions = async (sourceImageBase64: string, targetImageBase64: string): Promise<MorphResult> => {
   return callGemini('getMorphSuggestions', async () => {
        const prompt = `
...
export const getColorHarmonySuggestions = async (base64Image: string): Promise<ColorHarmonyResult> => {
    return callGemini('getColorHarmonySuggestions', async () => {
        const prompt = `
...
export const findNearbySalons = async (locationQuery: string): Promise<Salon[]> => {
    return callGemini('findNearbySalons', async () => {
        const prompt = `
